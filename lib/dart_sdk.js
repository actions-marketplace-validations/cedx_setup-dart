...
export class DartSdk {
	 ...
	 */
	async install() {
		let sdkDir = find("dart-sdk", this.version, this.architecture);
		if (!sdkDir || this.version == "latest") {
			const path = await this.download();
			this.version = (await readFile(join(path, "version"), "utf8")).trim();
			sdkDir = await cacheDir(path, "dart-sdk", this.version, this.architecture);
		}

		addPath(join(sdkDir, "bin"));
		return sdkDir;
	}
}
