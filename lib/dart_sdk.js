...
class DartSdk {
    ...
    async setup() {
        let skdDir = tool_cache_1.find('dart', this.version, this.architecture);
        if (!skdDir) {
            const output = await tool_cache_1.extractZip(await tool_cache_1.downloadTool(this.downloadUrl));
            skdDir = await tool_cache_1.cacheDir(path_1.join(output, 'dart-sdk'), 'dart', this.version, this.architecture);
        }
