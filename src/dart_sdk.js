...
export class DartSdk {
	 ...
	 * @param {DartSdkOptions} [options] An object providing values to initialize this instance.
	 */
	constructor(options = {}) {
		this.architecture = options.architecture ?? Architecture.x64;
		this.releaseChannel = options.releaseChannel ?? ReleaseChannel.stable;
	...
	get releaseUrl() {
		return new URL(format(
			DartSdk.#downloadUrlPattern,
			this.releaseChannel == ReleaseChannel.main ? "be/raw" : `${this.releaseChannel}/release`,
			this.version,
			platform == "win32" ? Platform.windows : platform == "darwin" ? Platform.macOS : Platform.linux,
			this.architecture
		));
	}
